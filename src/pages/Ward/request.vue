  ...
  <div class="content">
    ...
    <md-card>
      <md-card-header data-background-color="green">
        <h4 class="title">{{this.$route.params.id ? 'Sửa' : 'Thêm'}} phường/xã</h4>
        <p class="category">Điền thông tin phường/xã</p>
      </md-card-header>
      ...
      <md-card-content>
        <div class="md-layout">
          ...
          <div class="md-layout-item md-size-100 text-right">
            <md-button @click="submit" class="md-raised md-success">{{ this.$route.params.id ? 'Cập nhật' : "Thêm mới" }} phường/xã</md-button>
          </div>
        </div>
      </md-card-content>
    </md-card>
  ...
  methods:{
    submit() {
      if (this.$route.params.id) {
        ...
        const url = `/api/admin/api/v1/wards/${this.$route.params.id}`;
        axios.put(url, body)
        .then(response => {
          if (response.status === 200) {
            ...
            }).then((check) => { if(check.isConfirmed) this.$router.push('/ward')});
          }
        })
        .catch(error => {
      ...
      const url = '/api/admin/api/v1/wards';
      const body = this.dataRequest
      axios.post(url, body)
        .then(response => {
          ...
          if (response.status === 200) {
            this.$swal({
            title:'Thêm phường/xã thành công',
            icon:'success',
            confirmButtonText: 'OK'
            }).then((check) => { if(check.isConfirmed) this.$router.push('/ward')});
          }
