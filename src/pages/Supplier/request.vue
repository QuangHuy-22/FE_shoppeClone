  ...
  <div class="content">
    ...
    <md-card>
      <md-card-header data-background-color="green">
        <h4 class="title">{{this.$route.params.id ? 'Sửa' : 'Thêm'}} nhà cung cấp</h4>
        <p class="category">Điền thông tin nhà cung cấp</p>
      </md-card-header>
        ...
        <div class="md-layout">
          ...
          <div class="md-layout-item md-small-size-100 md-size-100">
            <md-field>
              ...
              <md-input v-model="dataRequest.phone" type="number"></md-input>
            </md-field>
          </div>
          <div class="md-layout-item md-small-size-100 md-size-100">
            <md-field>
          ...
          <div class="md-layout-item md-size-100 text-right">
            <md-button @click="submit" class="md-raised md-success">{{ this.$route.params.id ? 'Cập nhật' : "Thêm mới" }} nhà cung cấp</md-button>
          </div>
        </div>
      </md-card-content>
    </md-card>
  ...
  methods:{
    submit() {
      if (this.$route.params.id) {
        ...
        const url = `/api/admin/api/v1/suppliers/${this.$route.params.id}`;
        axios.put(url, body)
        .then(response => {
          if (response.status === 200) {
            ...
            }).then((check) => { if(check.isConfirmed) this.$router.push('/supplier')});
          }
        })
        .catch(error => {
      ...
      const url = '/api/admin/api/v1/suppliers';
      const body = this.dataRequest
      axios.post(url, body)
        .then(response => {
